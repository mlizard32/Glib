...
extern(C) nothrow void igImplSDLGL3_RenderDrawLists(ImDrawData* data)
{
...
void igImplSDL2GL3_Init(SDL_Window* window, bool install_callbacks)
{
	g_window = window;
    ...
    io.RenderDrawListsFn = &igImplSDLGL3_RenderDrawLists;
    io.SetClipboardTextFn = &igImplSDLGL3_SetClipboardText;
    io.GetClipboardTextFn = &igImplSDLGL3_GetClipboardText;
	/+#ifdef _MSC_VER
...
void igImplSDLGL3_CreateDeviceObjects()
{
	...
	igImplSDLGL3_CreateFontsTexture();
}

extern(C) nothrow const(char)* igImplSDLGL3_GetClipboardText()
{
   // return glfwGetClipboardString(g_window);
	return SDL_GetClipboardText();
}

extern(C) nothrow void igImplSDLGL3_SetClipboardText(const(char)* text)
{
   // glfwSetClipboardString(g_window, text);
	SDL_SetClipboardText(text);
}
/*
extern(C) nothrow void igImplGlfwGL3_MouseButtonCallback(GLFWwindow*, int button, int action, int )
{
    if (action == GLFW_PRESS && button >= 0 && button < 3)
        g_MousePressed[button] = true;
}

extern(C) nothrow void igImplGlfwGL3_ScrollCallback(GLFWwindow*, double , double yoffset)
{
...
*/
void igImplSDLGL3_CreateFontsTexture()
{
	ImGuiIO* io = igGetIO();
...
void igImplSDLGL3_NewFrame()
{
	if (!g_FontTexture)
		igImplSDLGL3_CreateDeviceObjects();

	auto io = igGetIO();

	io.DisplaySize = ImVec2(cast(float)720, cast(float)480);
	...
	io.MouseWheel = Input.mouseAxis[MouseAxisCodes.Scroll].getState;

	io.MouseDown[0] = Input.mouseBtns[MouseButtonCodes.Left].getState;
	io.MouseDown[1] = Input.mouseBtns[MouseButtonCodes.Right].getState;
	io.MouseDown[2] = Input.mouseBtns[MouseButtonCodes.Middle].getState;

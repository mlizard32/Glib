module Editor.TreeModelDragable;
/*
...
/**
*/
public class TreeModelDragable : ObjectG, TreeModelIF, TreeDragDestIF, TreeDragSourceIF
{
	...
	mixin TreeDragDestT!(GtkTreeModel);

	// add the TreeDragSource capabilities
	mixin TreeDragSourceT!(GtkTreeModel);
	...
	* Sets our main struct and passes it to the parent class
	*/
	public this (GtkTreeDragDest* gtkTreeModel, bool ownedRef = false)
	{
		super(cast(GObject*)gtkTreeModel, ownedRef);
		this.gtkTreeModel = cast(GtkTreeModel*) gtkTreeModel;
	}

	/**
	* Sets our main struct and passes it to the parent class
	*/
	public this (GtkTreeDragSource* gtkTreeModel, bool ownedRef = false)
	{
		super(cast(GObject*)gtkTreeModel, ownedRef);
		this.gtkTreeModel =cast(GtkTreeModel*) gtkTreeModel;
	}
	...
	extern(C)
	{
		...
		*/
		...
		static GType customTreeModelgetType()
		{
			...
			if (customTreeModelType == GType.INVALID)
			{
				...
				{
					cast(GInterfaceInitFunc) &customDragDestInit,
					null,
					null
				};
				...
				{
					cast(GInterfaceInitFunc) &customDragSourceInit,
					null,
					null
				};
				...
				Type.addInterfaceStatic (customTreeModelType, gtk_tree_drag_dest_get_type() /*GTK_TYPE_TREE_Drag_Dest*/, &dragDestInfo);
				Type.addInterfaceStatic (customTreeModelType, gtk_tree_drag_source_get_type() /*GTK_TYPE_TREE_Drag_Source*/, &dragSourceInfo);
			}
		...
		*/
		...
		static void customDragDestInit (GtkTreeDragDestIface *iface)
		{
			iface.dragDataReceived = &customDragDataReceived;
			iface.rowDropPossible = &customRowDropPossible;
		}

		static void customDragSourceInit (GtkTreeDragSourceIface *iface)
		{
			iface.rowDraggable = &customRowDraggable;
			iface.dragDataGet = &customDragDataGet;
			iface.dragDataDelete = &customDragDataDelete;
		}
		/**
		*
		* Params:
		...
		*/
		static int customDragDataReceived(GtkTreeDragDest* dragDest, GtkTreePath* dest, GtkSelectionData* selectionData) 
		{
			auto dd = ObjectG.getDObject!(TreeModelDragable)(dragDest);
			return dd.dragDataReceived(ObjectG.getDObject!(TreePath)(dest), ObjectG.getDObject!(SelectionData)(selectionData));
		}
		/**
		*
		* Params:
		...
		*/
		static int customRowDropPossible(GtkTreeDragDest* dragDest, GtkTreePath* destPath, GtkSelectionData* selectionData) 
		{
			auto dd = ObjectG.getDObject!(TreeModelDragable)(dragDest);
			return dd.rowDropPossible(ObjectG.getDObject!(TreePath)(destPath), ObjectG.getDObject!(SelectionData)(selectionData));
		}
		...
		*/
		static int customRowDraggable(GtkTreeDragSource* dragSource, GtkTreePath* path) 
		{
			auto ds = ObjectG.getDObject!(TreeModelDragable)(dragSource);
			return ds.rowDraggable(ObjectG.getDObject!(TreePath)(path));
		}
		...
		*/
		static int customDragDataGet(GtkTreeDragSource* dragSource, GtkTreePath* path, GtkSelectionData* selectionData)
		{
			auto ds = ObjectG.getDObject!(TreeModelDragable)(dragSource);
			return ds.dragDataGet(ObjectG.getDObject!(TreePath)(path), ObjectG.getDObject!(SelectionData)(selectionData));
		}
		/**
		*
		* Params:
		...
		*/
		static int customDragDataDelete(GtkTreeDragSource* dragSource, GtkTreePath* path)
		{
			auto ds = ObjectG.getDObject!(TreeModelDragable)(dragSource);
			return ds.dragDataDelete(ObjectG.getDObject!(TreePath)(path));
		}
		/*
		...
		*/
		...
		static int customTreeModelIterChildren(GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *parent)
		{
			...
			auto p = tm.iterChildren(ObjectG.getDObject!(TreeIter)(iter), ObjectG.getDObject!(TreeIter)(parent));
		...
		static int customTreeModelIterNthChild(GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *parent, int n)
		{
			...
			auto p = tm.iterNthChild(ObjectG.getDObject!(TreeIter)(iter), ObjectG.getDObject!(TreeIter)(parent), n);
		...
		static int customTreeModelIterParent(GtkTreeModel *tree_model, GtkTreeIter *iter, GtkTreeIter *child)
		{
			...
			auto p = tm.iterParent(ObjectG.getDObject!(TreeIter)(iter), ObjectG.getDObject!(TreeIter)(child));
